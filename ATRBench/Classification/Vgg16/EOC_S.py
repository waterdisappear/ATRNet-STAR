...
from HDANet.附图.废弃.byol_pytorch import default
...
def parameter_setting():
    ...
    parser.add_argument('--weights', type=str,
                        default=r"D:\Data\无人集群感知数据库_数据集分割\测试结果\Vgg16\model\EOC_scene.pth",
                        help='path to model weights (.pth) for test-only')
    parser.add_argument('--test_only', default=True,
                        help='only run evaluation using --weights, no training')
...
if __name__ == '__main__':
    ...
    # Use the training set mapping as the global label space (fix label mismatch across test folders).
